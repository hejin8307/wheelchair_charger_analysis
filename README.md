...
- 분석 배경
  ...
  - 자치구별 장애인 인구수 대비 설치된 충전소 개수에 불균형 존재
    ...
    - 평균 이하 자치구 중 전체 인구 대비 장애인구가 많은 6개 자치구(광진구, 금천구, 동대문구, 동작구, 성동구, 중구)를 분석 대상으로 선정  
<br/>
...
<img src="./image/결과1.PNG" width="700" />

<img src="./image/결과2.PNG" width="700" />

- 장애인 인구수 대비 급속충전소가 부족한 자치구에 충전소를 추가 설치하여 자치구별 충전소 격차 해소
  - 6개의 자치구의 입지 추천 지역에 신규 급속충전소 21개 추가 설치
  - 서울시 급속충전소 설치율이 이전에 비해 4.1% 증가
  - 충전소 추가 설치 이후 자치구별 장애인구 대비 충전소 격차가 41.34% 감소
  - 1개의 급속충전소 당 장애인이 사용할 수 있는 비율이 3.19% 증가(광진구, 금천구, 동대문구, 동작구, 성동구, 중구의 경우 각 34%, 31%, 7%, 16%, 14%, 27%로 증가)
